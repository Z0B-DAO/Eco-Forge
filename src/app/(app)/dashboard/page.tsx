...
function StatsRow() {
  ...
  const tierCount = milestones?.length ?? 7
  const overallPct = tierCount > 0
    ? ((currentTier + (range > 0 ? done / range : 0)) / tierCount) * 100
    : 0
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="rounded-xl border-[0.5px] border-white/60 bg-[#111111] p-5">
        ...
        <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-800">
          <div className="h-full rounded-full transition-all duration-500 bg-[#f97316]" style={{ width: `${overallPct}%` }} />
        </div>
        {milestones && (
