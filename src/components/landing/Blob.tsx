...
// ─── Shared blob displacement (same for all particle types) ───
...
void main() {
  ...
  // Surface attraction — almost all particles on the surface (water droplet)
  float attractionNoise = snoise(spiralDir * 1.5 + uTime * 0.08 + aPhase * 40.0);
  float attraction = smoothstep(-1.5, -0.2, attractionNoise); // ~98% attracted

  // Surface position: tight on the blob shell
  ...
  // Free particles stay close, not far out
...
// ─── Streak particle vertex shader ───
// These particles sit ON the blob surface and form visible lines
...
attribute float aLayerType; // 1=skin, 2=contour, 3=reflet, 4=halo
...
void main() {
  ...
  // Debug: map contour-halo (5) to halo (4) for visibility toggle
  ...
  // Flicker (skip for contour — must stay continuous)
  ...
  // Golden highlight on the right side (water bubble light refraction)
...
void main() {
  ...
  if (vLayerType > 1.5 && vLayerType < 3.5) {
    // Reflet: wide opaque core so dense particles merge into continuous lines
...
// ─── Streak Particles: placed along curves on sphere to form reflet lines ───
function StreakParticles({
  ...
  const { positions, alphas, sizes, phases, streakIds, depthLayers, layerTypes } = useMemo(() => {
    ...
    const sid: number[] = [];
    const dl: number[] = [];
    const lt: number[] = []; // layer type: 1=skin, 2=contour, 3=reflet, 4=halo

    let streakId = 0;

    // ── 1. SURFACE SKIN: dense tiny particles covering the sphere ──
    ...
    // ── Shared halo generator (used by contour & reflets) ──
    const haloLayers = 20;
    ...
    // ── 2. EXTRA RIM SKIN: dense particle band at equator for natural edge ──
    const rimExtraCount = 85000;
    for (let i = 0; i < rimExtraCount; i++) {
      const theta = Math.random() * Math.PI * 2;
      // Tight band ±15° around equator (the visible rim from camera)
    ...
    // ── 2b. ULTRA-RIM SKIN: ±5° band for crisp edge ──
    ...
    // ── 3. REFLET LINES: fewer, continuous lines inside ──
    const refletLines = [
      { count: 2, depth: 0.95, alphaCore: 1.0 },
      { count: 1, depth: 0.75, alphaCore: 0.8 },
    ];
    const refletPPL = 1200;

    for (const layer of refletLines) {
      for (let s = 0; s < layer.count; s++) {
        let theta = Math.random() * Math.PI * 2;
        let phi = Math.acos(2 * Math.random() - 1);
        const dTheta = (0.004 + Math.random() * 0.008) * (Math.random() > 0.5 ? 1 : -1);
        const dPhiBias = (Math.random() - 0.5) * 0.002;
        ...
        for (let p = 0; p < refletPPL; p++) {
          ...
          // Larger particles for continuous line overlap
          pos.push(x, y, z);
          alp.push(fade * layer.alphaCore);
          ...
          // Halo on every reflet particle (type 4 = reflet-halo)
          addHalo(x, y, z, fade, layer.alphaCore, layer.depth, streakId, 4.0);
...
function BlobScene() {
  ...
  useFrame((_, delta) => {
    ...
    if (timeRef.current - lastSpontRef.current > nextDelay.current) {
      ...
      nextDelay.current = 6 + Math.random() * 5;
    }
  ...
  // Debug: keyboard shortcuts to toggle layers
  // 0 = show all, 1 = Peau only, 2 = Contour only, 3 = Reflets only, 4 = Halo only
  useEffect(() => {
